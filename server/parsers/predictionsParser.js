    ...
    if ($subscribeBtn.length > 0) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        if (match) {
          const name = match[1];
          // Проверяем, что это не название команды
          const forbiddenWords = ['Сити', 'Мадрид', 'Барселона', 'Ливерпуль', 'Арсенал', 'Челси', 'Манчестер', 'Юнайтед', 'Бавария', 'Дортмунд', 'Ювентус', 'Милан', 'Интер', 'Реал', 'ПСЖ', 'Спартак', 'Динамо', 'Зенит', 'ЦСКА', 'Локомотив', 'Атлетико', 'Севилья', 'Наполи', 'Рома', 'Юн', 'Независимый', 'Эксперт', 'Каппер'];
