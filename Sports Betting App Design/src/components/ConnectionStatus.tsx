...
import { WifiOff, Shield, RefreshCw, MapPin } from 'lucide-react';
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl max-w-md w-full p-6 shadow-2xl">
        ...
        <div className="flex justify-center mb-4">
          <div className={`size-16 rounded-full flex items-center justify-center ${
            ...
            {issue === 'offline' ? (
              ...
              <MapPin className="size-8 text-orange-400" />
            )}
          </div>
        </div>
        ...
        <p className="text-zinc-400 text-center mb-6 leading-relaxed font-bold">
          {issue === 'offline' ? (
            ...
            <>
              Для безопасности и корректной работы приложения использование VPN запрещено. 
              Пожалуйста, отключите VPN и повторите попытку.
            </>
          ) : (
            <>
              Приложение доступно только для пользователей из России. 
              Ваш IP-адрес определен как находящийся за пределами РФ.
            </>
          )}
        </p>
            ...
            {issue === 'offline' ? (
              ...
              <>
                <li className="flex items-start gap-2 text-zinc-300">
                  ...
                  <span className="font-bold">Откройте настройки VPN на вашем устройстве</span>
                </li>
                <li className="flex items-start gap-2 text-zinc-300">
                  <span className="text-yellow-400 mt-0.5 font-bold">•</span>
                  <span className="text-[14px] font-bold">Отключите VPN полностью</span>
                </li>
                <li className="flex items-start gap-2 text-zinc-300">
                  <span className="text-yellow-400 mt-0.5 font-bold">•</span>
                  <span className="text-[15px] font-bold">После отключения нажмите "Повторить"</span>
                </li>
              </>
            ) : (
              <>
                <li className="flex items-start gap-2 text-zinc-300">
                  ...
                  <span>Убедитесь, что вы находитесь в России</span>
                </li>
                <li className="flex items-start gap-2 text-zinc-300">
                  <span className="text-orange-400 mt-0.5">•</span>
                  <span>Используйте российского интернет-провайдера</span>
                </li>
                <li className="flex items-start gap-2 text-zinc-300">
                  <span className="text-orange-400 mt-0.5">•</span>
                  <span>Отключите VPN, если он активен</span>
                </li>
              </>
            )}
          </ul>
        ...
        <p className="text-xs text-zinc-600 text-center mt-4 font-bold">
          {issue === 'offline' 
            ...
            : issue === 'vpn'
            ? 'Доступ к приложению возможен только без VPN'
            : 'Сервис доступен только на территории Российской Федерации'
          }
        </p>
      </div>
    </div>
